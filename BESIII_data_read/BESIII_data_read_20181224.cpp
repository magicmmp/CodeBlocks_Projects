...
protected:
    ...
    void UdpPacketToText(unsigned char*p,int udpBytesLeft)
    {
        bool udpLoop = true;
        int bufPos=0;
        while(udpLoop && udpBytesLeft>0)
        {
            if(CheckAndExtractPara(p+bufPos,udpBytesLeft)==0)
            {
                PrintToText();
                nUdpPacket++;
                bufPos=(tmp_para.HIT_COUNT<<3)+24;;
                udpBytesLeft=udpBytesLeft-bufPos;
            }
            else
            {
                udpLoop=false;
    ...
    int CheckAndExtractPara(unsigned char*dataBuff,int udpBytesLeft)
    {
        if(udpBytesLeft<24)
        {
            cout<<"packet number = "<<nUdpPacket<<", udpBytesLeft<24."<<endl;
            return -1;
        }
        for(int i=0;i<8;i=i+4)
        {
        ...
        int  nBytes;
        nBytes=(tmp_para.HIT_COUNT<<3)+24;
    ...
    void PrintToText()
    {
        outfile<<setw(56)<<" "<<setfill('*')<<setw(80)<<"*"<<setfill(' ')<<endl;
    ...
    {
        for(int i=9;i<rodSizes-4;i=i+1)
        {
            outfile<<hex;
