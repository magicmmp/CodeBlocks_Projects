...
public:
    ...
    int fileRead()
    {
        ...
        while(! infile.eof() && eventCount<10000)
        {
            infile.read((char*)buff,8);
            if(infile.fail())
            {
...
protected:
   ...
   void BinToText(int udpBytes)
    ...
    int CheckAndExtractPara(int nBytesLeft)
    {
        ...
        if(intbuff[0]>>29 != 6)
        {
            cout<<" Number of packet read = "<<nUdpPacket<<", header flag error."<<endl;
            return -1;
        }
        ...
        if(infile.fail())
        {
            cerr<<"cannot read udp data and tailer,"<<" Number of packet read = "<<nUdpPacket<<endl;
            return -1;
        }
        nBytes=nBytes+8;
        ...
        if(intbuff[idx]>>29 !=7)
        {
            cout<<" Number of packet read = "<<nUdpPacket<<", tailer flag error."<<endl;
            return -1;
        }
        ...
        {
            cout<<" Number of packet read = "<<nUdpPacket<<", intbuff[idx]>>24&0x7) !=tmp_para.LOCAL_L1_COUNT&0x7, error."<<endl;
            return -1;
        }
        ...
        if(intbuff[idx]>>28 !=4)
        {
            cout<<" Number of packet read = "<<nUdpPacket<<", udp sequence flag1 error."<<endl;
            return -1;
        }
    ...
    void PrintToText()
    {
        ...
        outfile<<setw(56)<<" "<<"###UDP packet NO.= "<<nUdpPacket<<"###"<<endl;
    ...
    void udpData(unsigned int idx1,unsigned int idx2)
    {
        ...
        {
            outfile<<hex;
            outfile<<setw(56)<<" "<<setw(8)<<intbuff[i];
...
private:
    int           nUdpPacket;/*记录是本文件的第几个打印出来的ROC UDP包了，便于出错提示*/
    para          tmp_para;
